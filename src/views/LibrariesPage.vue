  ...
  <ion-page>
    ...
    <ion-content>
      <ion-list class="reservation-type">
        <ion-item>
          ...
          >
            ...
            <ion-select-option
              v-for="(regionName, regionId) in regions"
              v-bind:key="regionId"
            >
              {{ regionId }}
            </ion-select-option>
      ...
      <div class="library-list">
        ...
        >
          <div
            class="region-libraries"
            v-if="(regionsShow == (regionID as unknown) || regionsShow == 'All')"
          >
            <h2 class="region-name">{{ regionID }}</h2>
            <div
              v-for="library in region"
            ...
            >
              ...
              <ion-button
                @click="openMessage(library)"
                v-bind:color="
                  library['availability'] == 'Reserve' ||
                  library['availability'] == 'Reserved'
                    ? 'success'
                    : library['availability'] == 'Notify me' ||
              ...
              <Transition>
                <div
                  v-if="
                    (reserveMessage || notificationMessage || requestMessage) &&
                    library.name == libraryMessage
                  "
                  class="message"
                >
                  <div class="message-wrapper">
                    <div v-if="reserveMessage" class="message-text">
                      ...
                      <p>
                        <span class="bolded">Library: </span>
                        {{ library["name"] }}
                      </p>
                    ...
                    <div v-if="notificationMessage" class="message-text">
                      <div class="message-title">
                        <h4>
                          Do you want to be notified when this book becomes
                          available again?
                        </h4>
                        <font-awesome-icon
                          @click="closeMessage()"
                    ...
                    <div v-if="requestMessage" class="message-text">
                      <div class="message-title">
                        <h4>
                          The selected book is currently unavailable at this
                          library. <br />
                          Do you wish to request it?
                        </h4>
                        <font-awesome-icon
                          @click="closeMessage()"
                          icon=" fa-xmark"
    ...
    methods: {
      ...
      confirmRequest(library) {
        if (this.reserveMessage) {
          ...
          setDoc(doc(this.reservedDB, "/" + this.bookId), {
        ...
        if (this.notificationMessage) {
          library.availability = "Confirmed";
          this.notificationMessage = false;
          setDoc(doc(this.notificationDB, "/" + this.bookId), {
            library: library.name,
          });
          return;
        }
