  ...
  export default defineComponent({
    ...
    methods: {
      ...
      setPath() {
        ...
        if (status === "") {
          this.router.replace({
            path: "/libraries/" + this.bookId,
            query: { name: this.book.title },
          });
        } else if (status === "Picked Up") {
...
<style scoped>
  ...
  .add-dropdown ion-select {
    --placeholder-opacity: 1;
    width: 100%;
    justify-content: center;
  }

  .add-dropdown ion-select::part(placeholder),
  .add-dropdown ion-select::part(text) {
    flex: 0 0 auto;
  }
